package example4.interfaceadapter
...
import example4.interfaceadapter.TaskPersistenceBehavior.{ Empty, Just, State }
  ...
  "Create" - {
    "In Empty state" - {
      "Should reply succeeded." in {
        ...
        assert(actual.event == Created(TaskId("1"), Subject("Test"), Status.Todo))
